...
class NFL_DATA_MODEL():
    def __init__(self, min_week, min_year, DVOA_Type):
        ...
        self.Regression_Model_Data_Path = f'{self.project_path}/Model Data/Regression {self.DVOA_Type} Model Data.csv'
        self.Classification_Model_Data_Path = f'{self.project_path}/Model Data/Classification {self.DVOA_Type} Model Data.csv'
...
##### HERE IS WHERE WE FIND THE MATCHUP CORRELATIONS #####
    ...
    def Get_Stat_Correlation(self):
        ...
        if self.year == 2019 and self.ego_score_prediction!= 0:
            self.Do_Graphing()      
...
##### HERE IS WHERE WE SAVE OUR NEW DATAFRAMES FOR THE USE OF MORE SOPHISTICATED MODELS #####
    ...
    def ReOrder_Cols(self):
        ...
        template_df = pd.read_csv(f'{self.project_path}/All_Model_Data_Template.csv')
...
##### HERE IS WHERE WE MAKE MATCHUP BASED BASIC PREDICTIONS ####
    ...
    def Save_New_Data(self):
        ...
        template_df = pd.read_csv(f'{self.project_path}/Total_Prediction_Data_Template.csv')
        cols = list(template_df)
        print(cols)
        #Re-Order Columns
